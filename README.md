...
- 🕰️ **Club history timeline** — 32 milestones, promotions/relegations and titles since 1939, filterable by category, cross-checked against the official club site and Wikipedia, with a dedicated "Trophies" highlight strip for the club's 4 titles
...
- 🚨 **Homepage alerts** — important upcoming matches (against FC Porto/Benfica/Sporting, or within 7 days) from real synced fixture data
...
- 💶 **Finance page** — real revenue/expenses/net result by season (2020/21–2024/25), manually sourced from Estoril Praia SAD's own public annual Reports and Accounts, with a link to the original PDF and a note per season on whether the figure is the final net result or the operating result
- 🗄️ A schema already modeling later phases (club history, market values, finance snapshots) so the project can grow past its MVP without a rewrite
...
│   │   │   ├── financas/        # Finance page (real Report & Accounts data by season)
...
- Homepage alerts — important matches use real, already-synced fixture data (opponent is FC Porto/Benfica/Sporting, or the match is within 7 days)
- Finance — real, 100% public data manually transcribed from Estoril Praia SAD's own annual Reports and Accounts (`estorilpraia.pt/institucional`), covering the 2020/21 through 2024/25 seasons. Three seasons (2021/22, 2022/23, 2023/24) report the final net result explicitly; two (2020/21, 2024/25) only had an operating result (before financing costs and taxes) stated in the source report — this distinction is called out per season, both on the page and in `messages/*.json`, rather than treating the two as interchangeable
...
- Fase 2 (history, comparator, news, market values) is complete. Of Fase 3, the staff area (login + news management), homepage alerts, CSV/PDF export and the finance page are shipped. Simulated physical performance and card-suspension alerts were built and then deliberately removed — this project shows **only real data**, never invented placeholders, even clearly labeled ones. xG, the tactical heatmap, and any other Fase 3 idea without a real free data source are left out for the same reason, rather than faked.
