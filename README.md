...
│   │   ├── simulated/            # Mock generators for simulated data (wearables/fatigue)
...
- Fase 2 (history, comparator, news, market values) is complete, and Fase 3 is in progress — the staff area (login + news management) and the simulated physical performance section are done; still to come are alerts, CSV/PDF export and a finance section. xG and the tactical heatmap are out of scope, since the football-data.org free tier doesn't provide shot/event data and the project stays 100% free-tier by design.
