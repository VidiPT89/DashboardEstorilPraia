...
- Fase 2 (history, comparator, news, market values) is complete, and Fase 3 is now complete too — staff area (login + news management), simulated physical performance, homepage alerts, CSV/PDF export and the finance page are all shipped. xG and the tactical heatmap are the only deliberate exceptions, out of scope since the football-data.org free tier doesn't provide shot/event data and the project stays 100% free-tier by design.
