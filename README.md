...
- 🚨 **Homepage alerts** — important upcoming matches (against FC Porto/Benfica/Sporting, or within 7 days) from real synced fixture data, plus a yellow/red-card watch list that's simulated, since card data isn't available on the free tier
...
- Homepage alerts — important matches use real, already-synced fixture data (opponent is FC Porto/Benfica/Sporting, or the match is within 7 days); yellow/red-card risk is 100% simulated by [`src/lib/simulated/cards.ts`](src/lib/simulated/cards.ts) with a simplified "3 yellows = 1-match ban" rule, since football-data.org has no card data on the free tier — documented here, without a badge on the homepage widget itself
