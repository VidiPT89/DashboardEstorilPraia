...
export async function getSimulatedCardAlerts(): Promise<PlayerCardAlert[]> {
  ...
  return rows
    .filter((row) => row.player !== null)
    .map((row) => {
      const value = row.value as unknown as { yellowCards: number; redCards: number };
      return {
        playerId: row.player!.id,
        playerName: row.player!.name,
        shirtNumber: row.player!.shirtNumber,
    ...
    .filter((entry) => entry.yellowCards >= SUSPENSION_THRESHOLD - 1 || entry.redCards > 0)
    .sort((a, b) => Number(b.isSuspended) - Number(a.isSuspended) || b.redCards - a.redCards || b.yellowCards - a.yellowCards);
}
